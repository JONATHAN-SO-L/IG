	...
	<div class="container">
		...
		<div class="container pass form-inline"><br>
			<form method="POST" action="#" class="form-inline">
				<div class="container">
					<strong><h3>CAMBIO DE CONTRASEÑA DEL GRUPO 5</h3></strong>
				</div>
...
if (isset($_POST['guardar'])) {
	...
	$sql = "SELECT contrasena FROM $table3 WHERE contrasena = '$pass_old' AND usuario = 'grupo5'";
	$con = mysqli_query($cn, $sql);
	$rel = mysqli_fetch_array($con);

	if ($rel['contrasena'] == $pass_old) {

		$query = "SELECT usuario FROM $table3 WHERE usuario = 'grupo5'";
		$action = mysqli_query ($cn, $query);
		$usr = mysqli_fetch_array ($action);

		if ($usr['usuario'] = 'grupo5') {
			...
			if ($pass_new == $rel['contrasena']) {
				...
				$cn->query("UPDATE usuarios SET contrasena = '$pass_new' WHERE contrasena = '$pass_old' AND usuario = 'grupo5'");
