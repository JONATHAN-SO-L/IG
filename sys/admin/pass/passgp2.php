...
if (isset($_POST['cambiar'])) {
	...
	$sql = "SELECT contraseña FROM $table4 WHERE contraseña = '$pass_old' AND usuario = 'grupo2'";
	$con = mysqli_query($cn, $sql);
	$rel = mysqli_fetch_array($con);

	if ($rel['contraseña'] == $pass_old) {

		$query = "SELECT usuario FROM $table4 WHERE usuario = 'grupo2'";
		...
		if ($usr['usuario'] = 'grupo2') {
			...
			if ($pass_new == $rel['contraseña']) {
				...
				$cn->query("UPDATE usuarios SET contraseña = '$pass_new' WHERE contraseña = '$pass_old' AND usuario = 'grupo2'");
