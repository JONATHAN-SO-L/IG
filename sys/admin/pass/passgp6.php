	...
	<div class="container">
		...
		<div class="container pass form-inline"><br>
			<form method="POST" action="#" class="form-inline">
				<div class="container">
					<strong><h3>CAMBIO DE CONTRASEÑA DEL GRUPO 6</h3></strong>
				</div>
...
if (isset($_POST['cambiar'])) {
	...
	$sql = "SELECT contraseña FROM $table4 WHERE contraseña = '$pass_old' AND usuario = 'grupo6'";
	$con = mysqli_query($cn, $sql);
	$rel = mysqli_fetch_array($con);

	if ($rel['contraseña'] == $pass_old) {

		$query = "SELECT usuario FROM $table4 WHERE usuario = 'grupo6'";
		$action = mysqli_query ($cn, $query);
		$usr = mysqli_fetch_array ($action);

		if ($usr['usuario'] = 'grupo6') {
			...
			if ($pass_new == $rel['contraseña']) {
				...
				$cn->query("UPDATE usuarios SET contraseña = '$pass_new' WHERE contraseña = '$pass_old' AND usuario = 'grupo6'");
