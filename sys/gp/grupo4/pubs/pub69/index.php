...
$val_info = "SELECT horas FROM $table2 WHERE id_pub = 69";
$sentencia = mysqli_query($cn, $val_info);
$resul = mysqli_fetch_array($sentencia);

switch ($resul['horas']) {
	case '':
	echo '<meta HTTP-EQUIV="REFRESH" CONTENT="0;URL=/ig/sys/gp/grupo4/pubs/pub69/registro.php">';
	break;

	default:
	$sql = "SELECT publicaciones, videos, horas, revisitas, cursos, comentarios FROM $table2 WHERE id_pub = 69";
	...
	while ($pub = mysqli_fetch_array($rel)) {
		...
		<a href="/ig/sys/gp/grupo4/pubs/pub69/mod.php"><button style="margin: 10px" type="sumbit" class="btn btn-warning btn-lg"><img id="modificar" src="/ig/assets/img/im.png"><strong class="modificar">MODIFICAR INFORME</strong></button></a>
		</div><br>
		</div>
		</div>
		</body>
